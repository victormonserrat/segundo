...
namespace edi 
{
	...
	{
		private:
			std::vector <edi::Vertex> vertex_;	//Vector of vertexes
			double **matrix_;					//Adjacency matrix of weights
			bool isDirected_;					//Is directed
			edi::Vertex *currentVertex_;		//cursor of vertexes
			edi::Edge *currentEdge_;			//cursor of edges
			unsigned int nVertexes_;			//number of vertexes
			unsigned int nEdges_;				//number of edges
			unsigned int capacity_;				//Capacity of graph (maximum value of
												//_numVertexes. _capacity >= _numVertexes)

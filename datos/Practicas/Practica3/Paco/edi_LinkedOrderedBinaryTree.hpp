...
public:
	...
	edi::LinkedOrderedBinaryTree <edi::Clave> *insertaFichero(edi::LinkedOrderedBinaryTree <edi::Clave> *t1, char *fich){
		std::ofstream f;
		f.open (fich, std::ofstream::out | std::ofstream::app);
		bool entrado = false;
		bool i = true;
		bool d = true;
		edi::LinkedOrderedBinaryTree <edi::Clave> *t2 = t1;
		edi::LinkedOrderedBinaryTree <edi::Clave> *t3 = t1;
		if (t1->hasLeftChild()){
			f << "(";
			f << *t1;
			f.flush();
			t2=insertaFichero(t1->leftSubTree(), fich);
			entrado=true;
		}
		else{
			i = false;
		...
		if (t1->hasRightChild()){
			...
			t3=insertaFichero(t1->rightSubTree(), fich); //t2
		}
		else{
			d = false;
		}
	...
	edi::LinkedOrderedBinaryTree <edi::Clave> *recogeFichero(edi::LinkedOrderedBinaryTree <edi::Clave> *t1, char *fich){
		char fichero [30]; //va a almacenar el nombre del fichero que nos pasan +Aux
		int size = (strlen(fich));	//almacena el tamaño de la variable fich
		std::string bash;	//variable para llamar a system
		
		bash = "cat " + (std::string)fich + " | tr -d ')(:' | sed 's/ /\\n/g' | sed '/^$/d' | sed 's/,/\\n/g'> " + (std::string)fich + "Aux && echo \"a\">> " + (std::string)fich + "Aux";
		...
		if (f.is_open()){
			...
			std::cout << "Ha habido un problema al abrir el archivo.";
			exit (-1);
		}
		f.close();
		bash="rm "+(std::string)fichero;
		system (bash.c_str());
		return (t1);
	}


	virtual int searchDni(const int &x)
	{
	...
	*/
	...
	//namespace edi{
	friend ofstream & operator<<(ofstream &out, const LinkedOrderedBinaryTree<Clave> &tree);

	friend ifstream & operator>>(ifstream &out, LinkedOrderedBinaryTree<Clave> &t);
	//}

...
namespace dis {
  ...
  /*!\fn Subject(std::vector<edi::Persona> const students)
     ...
     \pre None.
     \post None.*/
    Subject(std::vector<edi::Persona> students) {
  ...
  /*@}*/
  /*!\name Observers:@{*/
    dis::SortedDoublyLinkedList<edi::Persona> &getStudents() {return _students;}

    friend std::ofstream& operator<<(std::ofstream &out, dis::Subject &subject) {
      dis::SortedDoublyLinkedListNode<edi::Persona> *savedCurrent =
      subject.getStudents().getCurrent();
      dis::SortedDoublyLinkedListNode<edi::Persona> *aux;
      aux = subject.getStudents().getHead();
      while(aux != NULL) {
        out << aux->getItem();
        aux = aux->getNext();
      }
      subject.getStudents().setCurrent(savedCurrent);
      return(out);
    }
    friend std::ifstream& operator>>(std::ifstream &in, dis::Subject &subject) {
      edi::Persona p;
      char aux[50];
      while(!in.eof()) {
        in.getline(aux, 100, ' ');
  ...
  /*!\fn void modifyStudent(edi::Persona const &p)
    ...
    /*void flush() {
      ...
      _students.setCurrent(_students.getHead());
      while(_students.getCurrent() != NULL) {
        aux = _students.getCurrent()->getNext();
        delete _students.getCurrent();
        _students.setCurrent(aux);
      }
      _students.setHead(NULL);
      _students.setCurrent(NULL);
    }*/

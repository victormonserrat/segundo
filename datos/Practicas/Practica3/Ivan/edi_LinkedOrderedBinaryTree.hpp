...
public:
	...
	*/
	...
	std::string prefix(LinkedOrderedBinaryTree<ds::Key>* tree){
		std::string aux;
		if(tree != NULL){
				ds::Key key = tree->item();
				aux.append("(");
				aux.append(" ");
				aux.append(std::to_string(key.getDNI()));
				aux.append(",");
				aux.append(std::to_string(key.getPosition()));
				aux.append(" ");
				if(tree->hasLeftChild()){
					aux.append(prefix(tree->leftSubTree()));
				}
				else{
					aux.append(":");
				}
				...
				if(tree->hasRightChild()){
					aux.append(prefix(tree->rightSubTree()));
				}
				else{
					aux.append(":");
				}
	...
	void readFile(std::istream& in, LinkedOrderedBinaryTree<ds::Key>* tree){
			std::string aux;
			ds::Key auxKey;
			getline(in, aux, ' '); // (
			if(aux == "("){
				getline(in, aux, ','); // DNI
				auxKey.setDNI(atoi(aux.c_str()));
				getline(in, aux, ' '); // position
				auxKey.setPosition(atoi(aux.c_str()));
				tree->insert(auxKey);
				readFile(in, tree); //Left tree
				readFile(in, tree); //Right tree
				getline(in, aux, ')'); // )
				getline(in, aux, ' ');
			}
	}

	friend std::ostream& operator<< (std::ostream& out, LinkedOrderedBinaryTree<ds::Key>* t){
		LinkedBinaryTreeNode<ds::Key>* aux = t->root();
		t->setCurr(aux);
		out << t->prefix(t);
	...
	friend std::istream& operator>> (std::istream& in, LinkedOrderedBinaryTree<ds::Key>* t){
		t->readFile(in, t);
		return in;
	}

...
namespace edi{
	...
	*/
	...
	/*!
		\fn     std::ofstream & operator<<( std::ofstream &out, const edi::Persona &p)
		\brief  Escribe los datos de la Persona en un flujo de salida a fichero
		\note   Sobrecarga del operador de salida "<<"
		\param	stream Flujo de salida
		\param  p Persona, cuyos datos se van a escribir en el flujo de salida
		\return Flujo de salida en el que se han escrito los datos de la Persona
		\sa     operator>>
	*/
	std::ofstream & operator<<( std::ofstream &out, const edi::Persona &p)
	{
		out << p.nombre() << " " << p.apellido() << " " << p.dni() <<  std::endl;
		return out;
	};
	...
	*/
	...
	/*!
		\fn     std::ifstream &operator>>( std::ifstream &in, edi::Persona &p)
		\brief  Lee los datos de la Persona desde un flujo de entrada
		\note   Sobrecarga del operador de salida ">>"
		\param	stream Flujo de entrada
		\param  p Persona, cuyos datos se van a leer desde el flujo de entrada
		\return Flujo de salida con la persona recibida por fichero
		\sa     operator<<
	*/
	std::ifstream &operator>>( std::ifstream &in, edi::Persona &p)
	{
		char aux[30];
		int dni;
		in >> aux;
